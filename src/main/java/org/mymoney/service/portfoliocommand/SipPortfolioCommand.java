...
public class SipPortfolioCommand extends PortfolioCommand {
    @Override
    public void execute(ArrayList<String> args) {
        ArrayList<FundValueRequest> fundValueRequests = new ArrayList<FundValueRequest>();
        int fundIndex = 0;
        for (String str : args) {
            int value = Integer.parseInt(str);
            fundValueRequests.add(new FundValueRequest(FundType.values()[fundIndex++], value));
        }
        portfolioService.sip(fundValueRequests);
    }
}
